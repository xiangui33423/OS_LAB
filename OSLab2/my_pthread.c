...
#include <stdio.h>
	...
	static int atomic_test_and_set(volatile int* lock) {
	...
	void my_pthread_exit(void *value_ptr) {
		...
		thread_pool[current_thread].status = TERMINATED;
	...
	int my_pthread_join(my_pthread_t thread, void **value_ptr) {
		...
		// Deallocated any dynamic memory created when starting this thread
		tcb* target = get_thread_by_id(thread);
	...
	int my_pthread_mutex_lock(my_pthread_mutex_t *mutex) {
		...
		while (atomic_test_and_set(&mutex->lock) != 0) {
			thread_pool[current_thread].status = SUSPENDED;

			MUX_Node* new_node = (MUX_Node*) malloc(sizeof(MUX_Node));
			new_node->thread = &thread_pool[current_thread];
			new_node->next = mutex->waiting_list;
			mutex->waiting_list = new_node;

			if(swapcontext(&thread_pool[current_thread].context,  &scheduler_context) < 0)
			{
				return -1;
			}
		}
	...
	int my_pthread_mutex_unlock(my_pthread_mutex_t *mutex) {
		...
		MUX_Node* current = mutex->waiting_list;
		MUX_Node* prev = NULL;

		while (current != NULL) {
			current->thread->status = READY;

			#ifndef MLFQ
        	enqueue(ready_queue, current->thread);
        	#else
        	enqueue(mlfq->queues[current->thread->priority], current->thread);
        	#endif

			MUX_Node *tmp = current;
